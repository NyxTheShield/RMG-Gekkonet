...
void KailleraNetplayDialog::resizeEvent(QResizeEvent* event)
{
    ...
    if (m_tabWidget)
    {
        ...
        if (tabCount > 0)
        {
            int tabWidth = m_tabWidget->width() / tabCount;
            m_tabWidget->tabBar()->setStyleSheet(
                QString("QTabBar::tab { width: %1px; }").arg(tabWidth));
        }
    }
}
